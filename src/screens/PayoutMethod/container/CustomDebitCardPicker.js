...
export default function PickerModal(props) {
  ...
  const confirmHandler = () => {
    if (selectedValue === '') {
      if (props.pickOptions.length > 0) {
        const pickerItem = props.pickOptions[0];
        setSelectedValue(
          `${pickerItem.nickName} ${pickerItem.fundingSourceName}`,
        );
        props.onValueChange(
          `${pickerItem.nickName} ${pickerItem.fundingSourceName}`,
        );
  ...
  useEffect(() => {
    if (props.default) {
      if (props.pickOptions.length !== 0) {
        const pickerItem = props.pickOptions[0];
        const getFirstIndex = `${pickerItem.nickName} ${
          pickerItem.fundingSourceName
        }`;
  ...
  const items = props.pickOptions.map((item, id) => (
    <Picker.Item
      key={id}
      label={`${item.nickName} ${item.fundingSourceName}`}
      value={`${item.nickName} ${item.fundingSourceName}`}
    />
  ));
